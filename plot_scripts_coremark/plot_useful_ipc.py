...
for result in folders:
    ...
    for prefetcher_folder in prefetcher_folders: 
        ...
        with open(os.path.join(result_path, prefetcher_folder, "stats.txt"), "r") as file:
            ...
            for line in file:
                ...
                if match_useful:
                    coverage = float(match_useful.group(1))
                    if verbose: print(f"Coverage: {coverage}")
        
        ipc_list.append((prefetcher_folder, ipc))
        useful_list.append((prefetcher_folder, coverage))
    ...
    for prefetcher, ipc in ipc_list:
        ...
        useful_count = 0
        for useful_prefetcher, useful in useful_list:
            if useful_prefetcher == prefetcher:
                useful_count = useful
                break
        dat_file_data += f"{prefetcher_shorthand[prefetcher]} {ipc} {useful}\n"
        
    # Generate dat file  
    with open(os.path.join(result_path, f"{result}_useful.dat"), "w+") as file:
...
set output '{results_folder}/useful_ipc.png'
...
for i, result in enumerate(folders):
    ...
    plot_script += f"""
set title "{result}" 
plot '{results_folder}/{result}/{result}_useful.dat' using 2:xtic(1) title 'IPC' axes x1y1 linecolor rgb "#5397d4", '' using 3:xtic(1) title 'Useful' axes x1y2 linecolor rgb "#d9534f"
\n"""
...
with open(f"{results_folder}/plot_useful_ipc.gp", "w+") as file:
    file.write(plot_script)

# Call gnuplot
os.system(f"gnuplot {results_folder}/plot_useful_ipc.gp")
