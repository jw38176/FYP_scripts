...
for result in folders:
    ...
    for prefetcher_folder in prefetcher_folders: 
        ...
        with open(os.path.join(result_path, prefetcher_folder, "stats.txt"), "r") as file:
            ...
            for line in file:
                ...
                if match_late:
                    coverage = float(match_late.group(1))
                    if verbose: print(f"Coverage: {coverage}")
        
        ipc_list.append((prefetcher_folder, ipc))
        late_list.append((prefetcher_folder, coverage))
    ...
    for prefetcher, ipc in ipc_list:
        ...
        late_count = 0
        for late_prefetcher, late in late_list:
            if late_prefetcher == prefetcher:
                late_count = late
                break
        dat_file_data += f"{prefetcher_shorthand[prefetcher]} {ipc} {late}\n"
        
    # Generate dat file  
    with open(os.path.join(result_path, f"{result}_late.dat"), "w+") as file:
...
set output '{results_folder}/late_ipc.png'
...
for i, result in enumerate(folders):
    ...
    plot_script += f"""
set title "{result}" 
plot '{results_folder}/{result}/{result}_late.dat' using 2:xtic(1) title 'IPC' axes x1y1 linecolor rgb "#5397d4", '' using 3:xtic(1) title 'Late' axes x1y2 linecolor rgb "#d44ccf"
\n"""
...
with open(f"{results_folder}/plot_late_ipc.gp", "w+") as file:
    file.write(plot_script)

# Call gnuplot
os.system(f"gnuplot {results_folder}/plot_late_ipc.gp")
