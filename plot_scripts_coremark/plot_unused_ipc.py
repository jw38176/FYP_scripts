...
for result in folders:
    ...
    for prefetcher_folder in prefetcher_folders: 
        ...
        with open(os.path.join(result_path, prefetcher_folder, "stats.txt"), "r") as file:
            ...
            for line in file:
                ...
                if match_unused:
                    coverage = float(match_unused.group(1))
                    if verbose: print(f"Coverage: {coverage}")
        
        ipc_list.append((prefetcher_folder, ipc))
        unused_list.append((prefetcher_folder, coverage))
    ...
    for prefetcher, ipc in ipc_list:
        ...
        unused_count = 0
        for unused_prefetcher, unused in unused_list:
            if unused_prefetcher == prefetcher:
                unused_count = unused
                break
        dat_file_data += f"{prefetcher_shorthand[prefetcher]} {ipc} {unused}\n"
        
    # Generate dat file  
    with open(os.path.join(result_path, f"{result}_unused.dat"), "w+") as file:
...
set output '{results_folder}/unused_ipc.png'
...
for i, result in enumerate(folders):
    ...
    plot_script += f"""
set title "{result}" 
plot '{results_folder}/{result}/{result}_unused.dat' using 2:xtic(1) title 'IPC' axes x1y1 linecolor rgb "#5397d4", '' using 3:xtic(1) title 'Unused' axes x1y2 linecolor rgb "#53b567"
\n"""
...
with open(f"{results_folder}/plot_unused_ipc.gp", "w+") as file:
    file.write(plot_script)

# Call gnuplot
os.system(f"gnuplot {results_folder}/plot_unused_ipc.gp")
